...
import { cancelScrollAction, performEdgeScroll, isInEdge, canContainerBeScrolled } from '../helpers/edgescroller';
    ...
    onMouseMove (e) {
      ...
      if (this.dragStarted) {
        ...
        if (currEdgeSize) {
          ...
          // Iterate through all these nodes
          // The highest nodes (closest to window.body) will be scrolled first, then go up the list until the only
          //  scrollable element left is the current container
          for (let i = 0; i < nodes.length; i++) {
            const thisNode = nodes[i];
            
            // Check that the user's cursor is currently within an edge of this scrollable container
            const edgeParams = isInEdge(thisNode, x, y, currEdgeSize);
            if (!edgeParams) {
              continue;
            }
            ...
            const { canScroll } = canContainerBeScrolled(thisNode, viewportWidth, viewportHeight);
            if (!canScroll) {
              continue;
            }
            
            // Scroll the element un the X or Y direction
            performEdgeScroll(e, thisNode, x, y, currEdgeSize);
          }
        }
        else {
          cancelScrollAction();
