...
export function doDrop (inst, event) {
    const modeEventName = `on${inst.mode.charAt(0).toUpperCase() + inst.mode.substring(1)}`;
    
    console.log('drop event', inst.mode, event, inst, event.source.$el)
    inst.$emit('drop', event);
    // todo - $emit isn't working here
    // event.source.$emit(inst.mode, event);
    // todo - this works.......
    if (event.source.$attrs.hasOwnProperty(modeEventName)) {
        event.source.$attrs[modeEventName]()
    }
}
...
export default {
    ...
    computed: {
        compatibleMode() {
            if (this.dragInProgress) {
                const modeEventName = `on${this.mode.charAt(0).toUpperCase() + this.mode.substring(1)}`;
                console.log('formatted name', modeEventName, 'compatible mode:', this.mode === 'copy' || dnd.sourceListeners.hasOwnProperty(modeEventName));
                return this.mode === 'copy' || dnd.sourceListeners.hasOwnProperty(modeEventName);
            } else {
                return null;
            }
        },
    ...
    methods: {
         ...
         */
        candidate(type) { // todo - source: Vue
            return candidate(this, type)
        },
        createDragImage() {
            console.warn('called drag image creator', this.$refs);
            let image = 'source';
            if (this.$refs['drag-image']) {
                ...
                console.warn('has drag image', image)
            }
            return image;
        }
    },
